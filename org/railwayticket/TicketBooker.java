...
public class TicketBooker
{
    ...
    static Queue<Integer> waitingList = new LinkedList<>();//queue of WL passengers		-->only stores passenger-id
    static Queue<Integer> racList =  new LinkedList<>();//queu of RAC passengers   -->only stores passenger-id
    static List<Integer> bookedTicketList =  new ArrayList<>();//list of bookedticket passengers	-->only stores passenger-id
    
    
    //positioning of births like 1L 2L OR 1U 2U OR 1RAC 2 RAC 
    ...
    public void cancelTicket(int passengerId)
    {
        ...
        Passenger p = passengers.get(passengerId);  //-->stores class of passenger
        ...
        if(racList.size() > 0)
        {
            ...
            racList.remove(Integer.valueOf(passengerFromRAC.passengerId));//-->rack-list is queue 
            //In queue it is value based like set .
            availableRacTickets++;
            ...
            if(waitingList.size()>0)
            {
                ...
                passengerFromWaitingList.number = racPositions.get(0);/*the person moved to booked ticket leaves his rack position to the waiting-list member after giving
                removes the position from RAC list   hint**** there is only one position in the RAC so using get(0)..EX 1,2,3 1-moved so 2,3 and 1's position to WL */
